...
func CreateStarknetGenerateCommand() *cobra.Command {
	...
	starknetGenTypesCommand := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			headerTemplate := `// This file was generated by seer: https://github.com/moonstream-to/seer.
// seer version: %s
// seer command: seer starknet abigentypes --package %s
// Warning: Edit at your own risk. Any edits you make will NOT survive the next code generation.
`
			header := fmt.Sprintf(headerTemplate, version.SeerVersion, packageName)

			sections := []string{header}

			if packageName != "" {
				sections = append(sections, fmt.Sprintf("package %s", packageName))
			}
