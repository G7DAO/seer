...
func CreateCrawlerCommand() *cobra.Command {
	...
	var chain, baseDir, providerURI string
	var force bool
	...
	crawlerCmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			crawler := crawler.NewCrawler(chain, startBlock, endBlock, timeout, batchSize, confirmations, baseDir, force, providerURI)
	...
	crawlerCmd.Flags().StringVar(&providerURI, "provider-uri", "", "Any http node provider endpoint (default: '')")
