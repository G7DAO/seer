...
import (
	...
	"github.com/moonstream-to/seer/bindings/ImmutableCreate2Factory"
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if useSafe {
				...
				deployBytecode, err := generateOwnableERC721DeployBytecode(
					name_0,
					symbol,
					owner,
				)
				...
				err = DeployWithSafe(rpc, keyfile, password, common.HexToAddress(safeAddress), common.HexToAddress(txServiceBaseUrl), transactionOpts.Value, txServiceBaseUrl, deployBytecode, key.Address)
				if err != nil {
...
func generateOwnableERC721DeployBytecode(
	name_0 string,
	symbol string,
	owner common.Address,
) ([]byte, error) {
	abiPacked, err := OwnableERC721MetaData.GetAbi()
	if err != nil {
	...
	deployBytecode := append(common.FromHex(OwnableERC721MetaData.Bin), constructorArguments...)
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if useSafe {
				...
				transactionData, err := session.Approve(
				...
				err = CreateSafeProposal(rpc, keyfile, password, common.HexToAddress(safeAddress), contractAddress, transactionData.Data(), transactionOpts.Value, txServiceBaseUrl)
				if err != nil {
					return fmt.Errorf("failed to create Safe proposal: %v", err)
				}
...
func CreateMintCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if useSafe {
				// Generate transaction data
				transactionData, err := session.Mint(
...
func CreateRenounceOwnershipCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if useSafe {
				// Generate transaction data
				transactionData, err := session.RenounceOwnership()
				if err != nil {
					return fmt.Errorf("failed to generate transaction data: %v", err)
				}
...
func CreateSafeTransferFromCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if useSafe {
				// Generate transaction data
				transactionData, err := session.SafeTransferFrom(
...
func CreateSafeTransferFrom0Command() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if useSafe {
				// Generate transaction data
				transactionData, err := session.SafeTransferFrom0(
...
func CreateSetApprovalForAllCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if useSafe {
				// Generate transaction data
				transactionData, err := session.SetApprovalForAll(
...
func CreateTransferFromCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if useSafe {
				// Generate transaction data
				transactionData, err := session.TransferFrom(
...
func CreateTransferOwnershipCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if useSafe {
				// Generate transaction data
				transactionData, err := session.TransferOwnership(
...
func DeployWithSafe(rpcURL string, keyfile string, password string, safeAddress common.Address, factoryAddress common.Address, value *big.Int, txServiceBaseUrl string, deployBytecode []byte, from common.Address) error {
	// Generate salt
	salt, err := GenerateProperSalt(from)
	if err != nil {
...
func CreateSafeProposal(rpcURL string, keyfile string, password string, safeAddress common.Address, to common.Address, data []byte, value *big.Int, txServiceBaseUrl string) error {
	...
	safeInstance, err := GnosisSafe.NewGnosisSafe(to, client)
	if err != nil {
	...
	safeTxHash, err := CalculateSafeTxHash(to, safeTransactionData, chainID)
	if err != nil {
	...
	url := fmt.Sprintf(txServiceBaseUrl, chainID.String(), to.Hex())
	fmt.Println("url", url)
