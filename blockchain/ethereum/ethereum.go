...
func (c *Client) ParseBlocksWithTransactions(from, to *big.Int, debug bool, maxRequests int) ([]*EthereumBlock, error) {
...
func (c *Client) ParseEvents(from, to *big.Int, blocksCache map[uint64]indexer.BlockCache, debug bool) ([]*EthereumEventLog, error) {
...
func (c *Client) FetchAsProtoBlocks(from, to *big.Int, debug bool, maxRequests int) ([]proto.Message, []indexer.BlockIndex, []indexer.TransactionIndex, map[uint64]indexer.BlockCache, error) {
	...
	for bI, block := range parsedBlocks {
		...
		blocksIndex = append(blocksIndex, indexer.NewBlockIndex("ethereum",
			block.BlockNumber,
			block.Hash,
			block.Timestamp,
			block.ParentHash,
...
func (c *Client) FetchAsProtoEventsAndExtendBlock(from, to *big.Int, blocksCache map[uint64]indexer.BlockCache, debug bool) ([]proto.Message, []indexer.LogIndex, error) {
	...
	for eI, event := range parsedEvents {
		eventsProto = append(eventsProto, event) // Assuming event is a proto.Message
...
func ToProtoSingleBlock(obj *seer_common.BlockJson) *EthereumBlock {
...
func ToProtoSingleTransaction(obj *seer_common.TransactionJson) *EthereumTransaction {
	var accessList []*EthereumTransactionAccessList
	for _, al := range obj.AccessList {
		accessList = append(accessList, &EthereumTransactionAccessList{
			Address:     al.Address,
...
func ToEvenFromLogProto(obj *EthereumEventLog) *seer_common.EventJson {
...
func ToProtoSingleEventLog(obj *seer_common.EventJson) *EthereumEventLog {
...
func (c *Client) DecodeProtoTransactions(data []string) ([]*EthereumTransaction, error) {
...
func (c *Client) DecodeProtoEventsToLabels(rawData *bytes.Buffer, blocksCache map[uint64]uint64, abiMap map[string]map[string]map[string]string) ([]indexer.EventLabel, error) {
	var protoEvents []*EthereumEventLog
	for {
		protoEvent := &EthereumEventLog{}
		if unmErr := protodelim.UnmarshalFrom(rawData, protoEvent); unmErr != nil {
			if unmErr == io.EOF {
