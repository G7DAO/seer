...
func (c *Client) ParseBlocksWithTransactions(from, to *big.Int, debug bool, maxRequests int) ([]*B3SepoliaBlock, error) {
	...
	var parsedBlocks []*B3SepoliaBlock
...
func (c *Client) ParseEvents(from, to *big.Int, blocksCache map[uint64]indexer.BlockCache, debug bool) ([]*B3SepoliaEventLog, error) {
	...
	var parsedEvents []*B3SepoliaEventLog
...
func (c *Client) FetchAsProtoBlocksWithEvents(from, to *big.Int, debug bool, maxRequests int) ([]proto.Message, []indexer.BlockIndex, uint64, error) {
	...
	for bI, block := range blocks {
		...
		blocksIndex = append(blocksIndex, indexer.NewBlockIndex("b3_sepolia",
			block.BlockNumber,
			block.Hash,
			block.Timestamp,
			block.ParentHash,
...
func (c *Client) ProcessBlocksToBatch(msgs []proto.Message) (proto.Message, error) {
	var blocks []*B3SepoliaBlock
	for _, msg := range msgs {
		block, ok := msg.(*B3SepoliaBlock)
		if !ok {
			return nil, fmt.Errorf("failed to type assert proto.Message to *B3SepoliaBlock")
		}
		blocks = append(blocks, block)
	}

	return &B3SepoliaBlocksBatch{
		Blocks:      blocks,
		SeerVersion: version.SeerVersion,
	}, nil
}

func ToEntireBlocksBatchFromLogProto(obj *B3SepoliaBlocksBatch) *seer_common.BlocksBatchJson {
...
func ToProtoSingleBlock(obj *seer_common.BlockJson) *B3SepoliaBlock {
	return &B3SepoliaBlock{
...
func ToProtoSingleTransaction(obj *seer_common.TransactionJson) *B3SepoliaTransaction {
	var accessList []*B3SepoliaTransactionAccessList
	for _, al := range obj.AccessList {
		accessList = append(accessList, &B3SepoliaTransactionAccessList{
			Address:     al.Address,
			StorageKeys: al.StorageKeys,
		})
	}

	return &B3SepoliaTransaction{
		Hash:                 obj.Hash,
		BlockNumber:          fromHex(obj.BlockNumber).Uint64(),
...
func ToEvenFromLogProto(obj *B3SepoliaEventLog) *seer_common.EventJson {
...
func ToProtoSingleEventLog(obj *seer_common.EventJson) *B3SepoliaEventLog {
	return &B3SepoliaEventLog{
		Address:         obj.Address,
		Topics:          obj.Topics,
		Data:            obj.Data,
		BlockNumber:     fromHex(obj.BlockNumber).Uint64(),
...
func (c *Client) DecodeProtoEventLogs(data []string) ([]*B3SepoliaEventLog, error) {
	var events []*B3SepoliaEventLog
	for _, d := range data {
		var event B3SepoliaEventLog
...
func (c *Client) DecodeProtoTransactions(data []string) ([]*B3SepoliaTransaction, error) {
	var transactions []*B3SepoliaTransaction
	for _, d := range data {
		var transaction B3SepoliaTransaction
		base64Decoded, err := base64.StdEncoding.DecodeString(d)
		if err != nil {
			return nil, err
		}
...
func (c *Client) DecodeProtoBlocks(data []string) ([]*B3SepoliaBlock, error) {
	var blocks []*B3SepoliaBlock
	for _, d := range data {
		var block B3SepoliaBlock
		base64Decoded, err := base64.StdEncoding.DecodeString(d)
		if err != nil {
			return nil, err
		}
