...
func (c *Client) ProcessBlocksToBatch(msgs []proto.Message) (proto.Message, error) {
	...
	for _, msg := range msgs {
		block, ok := msg.(*B3Block)
		if !ok {
			return nil, fmt.Errorf("failed to type assert proto.Message to *B3Block")
		}
		blocks = append(blocks, block)
	}
...
func ToProtoSingleTransaction(obj *seer_common.TransactionJson) *B3Transaction {
	var accessList []*B3TransactionAccessList
	for _, al := range obj.AccessList {
		accessList = append(accessList, &B3TransactionAccessList{
			Address:     al.Address,
			StorageKeys: al.StorageKeys,
		})
	}

	return &B3Transaction{
		Hash:                 obj.Hash,
		BlockNumber:          fromHex(obj.BlockNumber).Uint64(),
...
func (c *Client) DecodeProtoEventLogs(data []string) ([]*B3EventLog, error) {
	var events []*B3EventLog
	for _, d := range data {
		var event B3EventLog
...
func (c *Client) DecodeProtoTransactions(data []string) ([]*B3Transaction, error) {
	var transactions []*B3Transaction
	for _, d := range data {
		var transaction B3Transaction
		base64Decoded, err := base64.StdEncoding.DecodeString(d)
		if err != nil {
			return nil, err
		}
...
func (c *Client) GetTransactionsLabels(startBlock uint64, endBlock uint64, abiMap map[string]map[string]map[string]string) ([]indexer.TransactionLabel, map[uint64]seer_common.BlockWithTransactions, error) {
	...
	// Get blocks in range
	blocks, err := c.FetchBlocksInRangeAsync(big.NewInt(int64(startBlock)), big.NewInt(int64(endBlock)), false, 20)
