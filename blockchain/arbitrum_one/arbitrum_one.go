...
func (c *Client) ParseBlocksAndTransactions(from, to *big.Int, debug bool, maxRequests int) ([]*ArbitrumOneBlock, []*ArbitrumOneTransaction, error) {
	...
	var parsedTransactions []*ArbitrumOneTransaction
...
func (c *Client) ParseEvents(from, to *big.Int, blocksCache map[uint64]indexer.BlockCache, debug bool) ([]*ArbitrumOneEventLog, error) {
...
func (c *Client) FetchAsProtoBlocks(from, to *big.Int, debug bool, maxRequests int) ([]proto.Message, []proto.Message, []indexer.BlockIndex, []indexer.TransactionIndex, map[uint64]indexer.BlockCache, error) {
	...
	for index, block := range parsedBlocks {
		...
		blockIndex = append(blockIndex, indexer.NewBlockIndex("arbitrum_one",
			block.BlockNumber,
			block.Hash,
			block.Timestamp,
			block.ParentHash,
...
func (c *Client) DecodeProtoEvents(rawData *bytes.Buffer) ([]*seer_common.EventJson, error) {
	...
	for {
		protoEvent := &ArbitrumOneEventLog{}
